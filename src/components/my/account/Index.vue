...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async submit() {
        const userName = await this.$contract.accountManager.getUserName(this.pictionConfig.account);
    ...
    async created() {
      this.name = await this.$contract.accountManager.getUserName(this.pictionConfig.account);
